...
export class PracticeService {
  ...
  async getStaffMembers() {
    try {
      ...
      // Fetch staff members - include all staff roles
      const { data, error } = await this.supabase
        .from('profiles')
        .select(`
          *,
          providers:providers!profiles_id_fkey(
            id,
            specialties,
            license_number,
            is_active,
        ...
        .in('role', ['surgeon', 'nurse', 'physical_therapist', 'provider', 'practice_admin', 'admin'])
      ...
      const transformedData = data?.map(staff => ({
        ...staff,
        providers: staff.providers?.[0] ? [{
          id: staff.providers[0].id,
          specialty: staff.providers[0].specialties?.[0] || 'General Practice',
          credentials: staff.providers[0].credentials || [],
          is_primary_surgeon: staff.providers[0].specialties?.[0] === 'Orthopedic Surgery'
        }] : []
      })) || [];
